...
export default async function ProfilePage({ params }: { params: Promise<{ username: string }> }) {
  ...
  return <main className="mx-auto flex min-h-screen max-w-2xl flex-col gap-6 px-4 py-8"><a href="/app" className="text-sm text-muted-foreground hover:text-foreground">← Back to feed</a><section className="rounded-2xl border border-border bg-card p-6"><div className="flex items-start justify-between gap-4"><div className="flex items-center gap-4"><Avatar name={profile.display_name} path={profile.avatar_path ? supabase.storage.from('avatars').getPublicUrl(profile.avatar_path).data.publicUrl : null} visible={profile.avatar_visible !== false} size="lg" /><div><h1 className="font-serif text-3xl">{profile.display_name}</h1><p className="text-sm text-muted-foreground">@{profile.username}</p></div></div>{profile.id !== user.id && <div className="flex items-center gap-2"><Link href={`/app/messages?user=${profile.id}`} className="rounded-full border border-border px-4 py-2 text-sm font-medium hover:bg-muted">Message</Link><FollowButton userId={profile.id} initialFollowing={Boolean(follow)} /></div>}</div>{profile.bio && <p className="mt-5 leading-7 text-muted-foreground">{profile.bio}</p>}</section><section className="flex flex-col gap-3">{(posts ?? []).map((post) => <article key={post.id} className="rounded-2xl border border-border bg-card p-5"><p className="whitespace-pre-wrap leading-7">{post.content}</p><p className="mt-4 text-xs text-muted-foreground">{new Date(post.created_at).toLocaleDateString()}</p></article>)}</section></main>
}
