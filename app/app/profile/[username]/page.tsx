...
export default async function ProfilePage({ params }: { params: Promise<{ username: string }> }) {
  ...
  if (!profile) notFound()
  const [{ data: posts }, { data: follow }, { data: friendship }] = await Promise.all([
    ...
    supabase.from('friendships').select('id, requester_id, addressee_id, status').or(`and(requester_id.eq.${user.id},addressee_id.eq.${profile.id}),and(requester_id.eq.${profile.id},addressee_id.eq.${user.id})`).maybeSingle(),
  ])
  return <main className="mx-auto flex min-h-screen max-w-2xl flex-col gap-6 px-4 py-8"><a href="/app" className="text-sm text-muted-foreground hover:text-foreground">← Back to feed</a><section className="rounded-2xl border border-border bg-card p-6"><div className="flex items-start justify-between gap-4"><div className="flex items-center gap-4"><Avatar name={profile.display_name} path={profile.avatar_path ? supabase.storage.from('avatars').getPublicUrl(profile.avatar_path).data.publicUrl : null} visible={profile.avatar_visible !== false} size="lg" /><div><h1 className="font-serif text-3xl">{profile.display_name}</h1><p className="text-sm text-muted-foreground">@{profile.username}</p></div></div>{profile.id !== user.id && <div className="flex items-center gap-2"><Link prefetch={false} href={`/app/messages?user=${encodeURIComponent(profile.id)}`} className="rounded-full border border-border px-4 py-2 text-sm font-medium hover:bg-muted">Message</Link><FollowButton userId={profile.id} initialFollowing={Boolean(follow)} /><FriendButton userId={profile.id} friendshipId={friendship?.id} initialStatus={friendship?.status === 'accepted' || (friendship?.status === 'pending' && friendship.requester_id === user.id) ? friendship.status : null} /><SafetyControls userId={profile.id} /></div>}</div>{profile.bio && <p className="mt-5 leading-7 text-muted-foreground">{profile.bio}</p>}</section><section className="flex flex-col gap-3">{(posts ?? []).map((post) => <article key={post.id} className="rounded-2xl border border-border bg-card p-5"><p className="whitespace-pre-wrap leading-7">{post.content}</p><p className="mt-4 text-xs text-muted-foreground">{new Date(post.created_at).toLocaleDateString()}</p></article>)}</section></main>
}
