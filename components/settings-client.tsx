...
type Preferences = { likes: boolean; follows: boolean; messages: boolean; comments: boolean; friend_requests: boolean; status_views: boolean } | null
...
function Notifications({ preferences, pending, run }: { preferences: Preferences; pending: boolean; run: Run }) { return <Panel title="Notification preferences" text="Choose which activity appears in your notifications."><form onSubmit={event => { event.preventDefault(); run(updateNotificationPreferences, new FormData(event.currentTarget)) }} className="flex flex-col gap-3">{[['likes', 'Likes on your posts'], ['follows', 'New followers'], ['messages', 'New messages'], ['comments', 'Comments on your posts'], ['friend_requests', 'Friend requests'], ['status_views', 'Status views']].map(([name, label]) => <label key={name} className="flex justify-between rounded-xl border border-border px-4 py-3 text-sm">{label}<input name={name} type="checkbox" defaultChecked={preferences?.[name as keyof NonNullable<Preferences>] ?? true} /></label>)}<Submit pending={pending}>Save notifications</Submit></form></Panel> }
