...
type Run = (fn: (data: FormData) => Promise<{ error?: string }>, form: HTMLFormElement) => void
...
export function SettingsClient({ profile, email, preferences }: Props) {
  ...
  const run: Run = (fn, form) => startTransition(async () => setMessage((await fn(new FormData(form))).error ?? 'Changes saved.'))
