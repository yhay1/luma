...
export function StatusViewer({ statuses, start, onClose }: { statuses: Status[]; start: number; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/95 p-4" role="dialog" aria-label="Status viewer"><div className="relative flex h-full w-full max-w-lg flex-col justify-between overflow-hidden rounded-3xl border border-border bg-card"><div className="absolute inset-x-4 top-4 z-10 flex gap-1">{statuses.map((_, i) => <span key={i} className="h-1 flex-1 rounded-full bg-muted"><span className={`block h-full rounded-full bg-primary ${i < index ? 'w-full' : i === index ? 'w-1/2' : 'w-0'}`} /></span>)}</div><div className="relative flex flex-1 items-center justify-center"><img src={current.url} alt={`Status by ${current.displayName}`} className="max-h-full max-w-full object-contain" /><button onClick={onClose} aria-label="Close status" className="absolute right-4 top-10 rounded-full bg-background/70 p-2"><X className="size-5" /></button><button onClick={() => setIndex((i) => Math.max(0, i - 1))} aria-label="Previous status" className="absolute left-2 rounded-full bg-background/70 p-2"><ChevronLeft className="size-5" /></button><button onClick={() => index === statuses.length - 1 ? onClose() : setIndex((i) => i + 1)} aria-label="Next status" className="absolute right-2 rounded-full bg-background/70 p-2"><ChevronRight className="size-5" /></button></div><div className="flex items-end justify-between gap-4 p-5"><div><p className="font-medium">{current.displayName}</p>{current.caption && <p className="mt-1 text-sm text-muted-foreground">{current.caption}</p>}</div><button onClick={() => setPaused((v) => !v)} aria-label={paused ? 'Resume status' : 'Pause status'} className="rounded-full bg-muted p-2">{paused ? <Play className="size-4" /> : <Pause className="size-4" />}</button></div></div></div>
}
