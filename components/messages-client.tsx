...
import { markConversationRead, sendMessage, startConversation } from '@/lib/social/chat-actions'
...
export function MessagesClient({ currentUserId, conversations }: { currentUserId: string; conversations: Conversation[] }) {
  ...
  async function openChat(item: Conversation) {
    ...
    if (!item.conversationId) {
      ...
      if (!result.conversationId) return setError(result.error ?? 'Unable to start conversation.')
